...
type symbol_table = {
  vars: (string * var_decl_checked * var_type) list;
...
let rec check_id (env : symbol_table) id =
...
and check_binop (env : symbol_table) (binop: Ast.expr) = match binop with
...
let check_fdecl (env : symbol_table) (f : Ast.func_decl) = match f.fname with
  ...
  | _ -> sast_fdecl env f (find_rtype f.body)
